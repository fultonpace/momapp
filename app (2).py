...
# ── HTML block renderer (bypasses Streamlit's HTML sanitizer) ─────────────────
def render_html(html: str, height: int = 100):
    """Render arbitrary styled HTML via an iframe component."""
    # Wrap in a minimal page that inherits nothing from Streamlit
    full = f"""<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8">
...
<style>
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  body {{ background: transparent; font-family: 'Cormorant Garamond', Georgia, serif;
          color: #e8e4dc; overflow: hidden; }}
</style>
</head>
<body>{html}</body>
</html>"""
...
def main():
    ...
    # Title — these class-only divs are safe for st.markdown
    ...
    with col1:
        ...
        valence = st.selectbox(
            "Valence", VALENCE, index=0, label_visibility="collapsed", key="valence",
        )

    with col2:
        st.markdown('<div class="dim-label">Ⅱ — Arousal</div>', unsafe_allow_html=True)
        arousal = st.selectbox(
            "Arousal", AROUSAL, index=0, label_visibility="collapsed", key="arousal",
        )

    with col3:
        st.markdown('<div class="dim-label">Ⅲ — Stance</div>', unsafe_allow_html=True)
        stance = st.selectbox(
            "Stance", STANCE, index=0, label_visibility="collapsed", key="stance",
        )
    ...
    # ── Chips + palette name via render_html ───────────────────────────────────
    render_html(f"""
<style>
  .emotion-chips {{ display:flex; gap:0.6rem; flex-wrap:wrap; margin-bottom:0.8rem; }}
  .chip {{
    font-family:'DM Mono',monospace; font-size:0.62rem; letter-spacing:0.18em;
    text-transform:uppercase; padding:4px 10px;
    border:1px solid rgba(255,255,255,0.07); border-radius:1px; color:rgba(255,255,255,0.35);
  }}
  .dim-v {{ border-color:rgba(200,169,110,0.4); color:rgba(200,169,110,0.85); }}
  .dim-a {{ border-color:rgba(110,160,200,0.4); color:rgba(110,160,200,0.85); }}
  .dim-s {{ border-color:rgba(160,200,110,0.4); color:rgba(160,200,110,0.85); }}
  .palette-name {{
    font-family:'Cormorant Garamond',serif; font-size:1.4rem; font-style:italic;
    font-weight:300; color:rgba(232,228,220,0.45); letter-spacing:0.06em; margin-top:0.5rem;
  }}
</style>
...
<div class="palette-name">"{palette_name(valence, arousal, stance)}"</div>
""", height=90)

    # ── Swatch strip ───────────────────────────────────────────────────────────
    swatch_parts = []
    for sw in swatches:
        swatch_parts.append(f"""
...
<style>
  .palette-row {{
    display:flex; width:100%; border-radius:4px; overflow:hidden;
    box-shadow:0 8px 48px rgba(0,0,0,0.55);
  }}
  .swatch {{
    flex:1; height:220px; display:flex; flex-direction:column;
    justify-content:flex-end; padding:12px 10px;
    transition:flex 0.35s ease; cursor:default;
  }}
  .swatch:hover {{ flex:1.7; }}
  .swatch-hex {{
    font-family:'DM Mono',monospace; font-size:0.65rem;
    letter-spacing:0.1em; opacity:0.9; margin-bottom:3px; text-transform:uppercase;
  }}
  .swatch-hsl {{
...
<div class="palette-row">{"".join(swatch_parts)}</div>
""", height=240)

    # ── Hex reference strip ────────────────────────────────────────────────────
    hex_pills = "".join(
        f'<span style="font-family:\'DM Mono\',monospace; font-size:0.65rem; '
        f'letter-spacing:0.12em; padding:5px 10px; border-radius:2px; '
        f'border:1px solid rgba(255,255,255,0.08); color:rgba(232,228,220,0.45); '
        f'text-transform:uppercase; background:{sw["hex"]}20;">{sw["hex"]}</span>'
        for sw in swatches
    )
    render_html(f"""
<style>
  ...
  .hex-grid {{ display:flex; gap:0.4rem; flex-wrap:wrap; }}
</style>
<div class="label">Hex Values</div>
<div class="hex-grid">{hex_pills}</div>
""", height=80)
...
<style>
  ...
  p {{
    ...
    font-weight:300; line-height:1.75; color:rgba(232,228,220,0.62);
    max-width:760px; font-style:italic;
  }}
</style>
<div class="label">Emotional Profile</div>
<p>{narrative}</p>
""", height=130)
...
<style>
  ...
  p {
    ...
    line-height:1.85; color:rgba(232,228,220,0.58); font-weight:300;
    margin-bottom:1rem;
  }
  strong { font-weight:400; letter-spacing:0.08em; }
  .v { color:#c8a96e; }
  .a { color:#6ea0c8; }
  .s { color:#a0c86e; }
</style>
...
<p><strong class="v">Ⅰ — VALENCE</strong> &nbsp;(hedonic tone) governs hue temperature
and the overall saturation envelope. Positive valence pulls toward warm amber and golden
yellows; negative valence shifts the dominant hue family toward cool blues, desaturated
grays, and compressed chroma.</p>

<p><strong class="a">Ⅱ — AROUSAL</strong> &nbsp;(activation energy) controls lightness
contrast, saturation intensity, and color spread across the palette. High arousal produces
sharp value contrasts and fully saturated anchors; low arousal compresses values toward
mid-gray and softens saturation throughout.</p>

<p><strong class="s">Ⅲ — STANCE</strong> &nbsp;(relational posture) modulates harmony type
and boundary behavior between swatches. Open stances favor analogous harmonies with soft
edge transitions; closed or defensive stances favor complementary or split-complementary
structures with higher inter-swatch contrast.</p>

<p>Each combination seeds a deterministic function via MD5 hash — the same trio of
adjectives always yields the same palette, making ChromaLogic palettes reproducible
and citable as named emotional states.</p>
""", height=400)
